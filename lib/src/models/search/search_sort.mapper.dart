// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// dart format off
// ignore_for_file: type=lint
// ignore_for_file: invalid_use_of_protected_member
// ignore_for_file: unused_element, unnecessary_cast, override_on_non_overriding_member
// ignore_for_file: strict_raw_type, inference_failure_on_untyped_parameter

part of 'search_sort.dart';

class SearchSortMapper extends EnumMapper<SearchSort> {
  SearchSortMapper._();

  static SearchSortMapper? _instance;
  static SearchSortMapper ensureInitialized() {
    if (_instance == null) {
      MapperContainer.globals.use(_instance = SearchSortMapper._());
    }
    return _instance!;
  }

  static SearchSort fromValue(dynamic value) {
    ensureInitialized();
    return MapperContainer.globals.fromValue(value);
  }

  @override
  SearchSort decode(dynamic value) {
    switch (value) {
      case r'recent':
        return SearchSort.recent;
      case r'popular':
        return SearchSort.popular;
      case r'popularMonth':
        return SearchSort.popularMonth;
      case r'popularWeek':
        return SearchSort.popularWeek;
      case r'popularToday':
        return SearchSort.popularToday;
      default:
        throw MapperException.unknownEnumValue(value);
    }
  }

  @override
  dynamic encode(SearchSort self) {
    switch (self) {
      case SearchSort.recent:
        return r'recent';
      case SearchSort.popular:
        return r'popular';
      case SearchSort.popularMonth:
        return r'popularMonth';
      case SearchSort.popularWeek:
        return r'popularWeek';
      case SearchSort.popularToday:
        return r'popularToday';
    }
  }
}

extension SearchSortMapperExtension on SearchSort {
  String toValue() {
    SearchSortMapper.ensureInitialized();
    return MapperContainer.globals.toValue<SearchSort>(this) as String;
  }
}

